...
eof_time_series_H_avg = fp.transform(
    data_H_avg_trfm.values
)  # project DECK averaged data onto EOF
eof_time_series_era5 = fp.transform(data_era5_trfm.values)  # project era5 onto EOF
...
# project the pre-industrial data onto the EOFs
...
eof_ts = eof_time_series_H_avg[:, eof_num].copy()

eof_ts_PI = np.array(
    [
        eof_time_series_pi_data[ii][:, eof_num]
        for ii in range(len(eof_time_series_pi_data))
    ]
).T
...
# Statistical significance of the trend analysis
# Try using overlapping data
